...
> **문제**
  ...
  > N*N의 격자판이 주어지면 각 행의 합, 각 열의 합, 두 대각선의 합 중 가장 큰 합을 출력하는 프로그램을 작성하시오.
...
> **설명**
  ```
  1. 첫 번째 for문에서는 각 행의 합과 열의 합을 구한 후에 나온 합의 최대 값을 변수 max에 할당합니다.
  2. 두 번째 for문에서는 두 대각선의 합을 구하고 앞서 구한 최대 값과 비교하여 더욱 큰 수를 변수 max에 할당합니다.
